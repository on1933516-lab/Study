...
| **On After Fetch** | ・データ取得完了直後（Aggregate / Data Action） | 複数回（取得ごと） | ・取得データの加工<br>・集計や計算処理<br>・ウィジェット用データの整形 | ・次の再描画を誘発しない処理にする<br>・重い処理や副作用は注意 |
...
# ■ ブロックイベント（Block Events）

ブロックは画面と同様のイベントを持つが、  
**ブロック特有のイベントとして OnParametersChanged が用意されている。**
...
- On Destroy
- On After Fetch
- **On Parameters Changed（ブロック特有）**

## ■ On Parameters Changed

### トリガーされるタイミング
- 親（画面または親ブロック）が  
  **ブロックの入力パラメータのいずれかを変更したとき**

### 実行回数
- 入力パラメータが変更されるたびに実行される

### 目的
- 入力パラメータの変更に応じて、ブロック内部の状態を更新する

### 主な用途
- 関連データの再取得
- フィルタ条件の変更
- 集計・計算などの処理
- ブロック内 Aggregate / Data Action の更新
